...
import { Search, Filter, X, MapPin, Mail, Phone, Briefcase, Star, TrendingUp } from 'lucide-react'
...
const dummyAgents: Agent[] = [
  { id: '1', name: 'Sarah Travel', email: 'sarah@agency.com', phone: '+1-555-0201', location: 'New York', agency: 'TravelPro', bookings: 156, revenue: 45000, rating: 4.8, status: 'active' },
  { id: '2', name: 'Mike Journey', email: 'mike@agency.com', phone: '+1-555-0202', location: 'Miami', agency: 'VoyageWide', bookings: 89, revenue: 28500, rating: 4.5, status: 'active' },
  { id: '3', name: 'Lisa Adventure', email: 'lisa@agency.com', phone: '+1-555-0203', location: 'Los Angeles', agency: 'GlobalTours', bookings: 203, revenue: 62000, rating: 4.9, status: 'active' },
  { id: '4', name: 'Tom Explorer', email: 'tom@agency.com', phone: '+1-555-0204', location: 'Chicago', agency: 'PathFinder', bookings: 67, revenue: 18000, rating: 4.2, status: 'inactive' },
  { id: '5', name: 'Emma Vacation', email: 'emma@agency.com', phone: '+1-555-0205', location: 'Denver', agency: 'TravelPro', bookings: 145, revenue: 41000, rating: 4.7, status: 'active' },
]
...
export function AgentsSection() {
  ...
  const filteredAgents = useMemo(() => {
    return dummyAgents.filter((agent) => {
      ...
      const matchesLocation = !locationFilter || agent.location === locationFilter
  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
        ...
        {filteredAgents.length > 0 ? (
          <div className="space-y-3">
            {filteredAgents.map((agent) => (
              <Card key={agent.id} className="p-4 hover:shadow-md transition cursor-pointer border border-border">
                ...
                <div className="grid grid-cols-4 gap-2 pt-3 border-t border-border text-sm">
                  <div>
                    ...
                    <p className="font-semibold text-foreground">{agent.bookings}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground text-xs">Revenue</p>
                    <p className="font-semibold text-foreground">${(agent.revenue / 1000).toFixed(1)}k</p>
                  </div>
                  <div>
      ...
      <div className="border-t border-border pt-4 text-sm text-muted-foreground">
        <p>
          Showing <span className="font-semibold text-foreground">{filteredAgents.length}</span> of <span className="font-semibold text-foreground">{dummyAgents.length}</span> agents
        </p>
      </div>
    </div>
  )
